...
object TopNStatJob2 {
  ...
  def videoAccessTopNStat(spark: SparkSession, commonDf: DataFrame) = {
    ...
    val videoAccessTopNDf = commonDf.groupBy("day", "cmsId").agg(count("cmsId").as("times"))
                                    .orderBy($"times".desc)
...
//      "group by day,cmsId order by times desc")
  ...
  def videoTrafficsTopNStat(spark: SparkSession, commonDf: DataFrame) = {
    ...
    val videoTrafficsTopNDf = commonDf.groupBy("day", "cmsId").agg(sum("traffic").as("traffics"))
